...
# create a new data set with only the 100 genes with the highest variation
genes.var <- apply(dat, 2, var)
# calculate the variance for each column (dim2)
# variation of genes, see ?apply
# how much variation do we have for each gene
genes.var.select <- order(-genes.var)[1:100]
# see ?order
# order in ascending order and extract the indices
...
# why looking at genes with higher variation we are able to predict different cell
...
# the improvement I have in prediction using data.s is not due to the reduction
